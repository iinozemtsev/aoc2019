...
class Amplifiers {
  Memory program;
  Amplifiers(this.program);
  int thrustersOutput(List<int> phases) {
    var output = 0;
    for (var phase in phases) {
      var computer = Computer(Memory(program.state), Input([phase, output]))
        ..run();
      output = computer.output.output.single;
    }
    return output;
  }

  int loopedThrustersOutput(List<int> phases) {
    var computers =
        phases.map((p) => Computer(Memory(program.state), Input([p]))).toList();
    var output = 0;
    while (true) {
      expect(output, greaterThan(-1000000000));
      var someStopped = false;
      for (var computer in computers) {
        if (computer.state != State.stopped) {
          computer.input.input.add(output);
          computer.run();
          output = computer.output.output.last;
        } else {
          someStopped = true;
        }
      }
      if (someStopped) {
        if (!computers.every((c) => c.state == State.stopped)) {
          throw ArgumentError('Not all computers have stopped');
        }
        break;
      }
    }
  ...
  List<int> bestPhase() {
    var result = phases.first;
    var resultOutput = -100500;
    for (var p in phases) {
      var output = thrustersOutput(p);
  ...
  List<int> bestLoopingPhase() {
    var result = loopingPhases.first;
    var resultOutput = -100500;
    for (var p in loopingPhases) {
      var output = loopedThrustersOutput(p);
      if (output > resultOutput) {
        result = p;
        resultOutput = output;
      }
...
List<int> toDigits(int number, int minDigits) {
  var result = <int>[];
  while (number > 0 || result.length < minDigits) {
...
var phases = List.generate(100000, (i) => toDigits(i, 5))
    .where((digits) => digits.every((digit) => digit < 5))
    .where((digits) => digits.length == digits.toSet().length)
    .toSet();

var loopingPhases = List.generate(100000, (i) => toDigits(i, 5))
    .where((digits) => digits.every((digit) => digit > 4))
    .where((digits) => digits.length == digits.toSet().length)
    .toSet();
...
void main() {
  group('part1', () {
    ...
    test('task', () {
      print(Amplifiers(Memory(input)).bestPhase());
    });
  });
  ...
  group('part2', () {
    ...
    test('task', () {
      print(Amplifiers(Memory(input)).bestLoopingPhase());
    });
  });
}
