...
D directly orbits C and indirectly orbits B and COM, a total of 3 orbits. L
directly orbits K and indirectly orbits J, E, D, C, B, and COM, a total of 7
orbits. COM orbits nothing. The total number of direct and indirect orbits in
this example is 42.
...
class Planet {
  String name;
  Planet parent;
  List<Planet> children = <Planet>[];
  Planet(this.name);

  int get totalOrbits => path.length - 1;

  List<String> get path => parent == null ? [name] : (parent.path..add(name));

  @override
  String toString() => _print('');

  String _print(String indent) {
    var sb = StringBuffer();
    sb.writeln(indent + name);
    for (var child in children) {
      sb.write(child._print(indent + ' '));
    }
    return sb.toString();
  }
}
...
class StellarMap {
  final _planets = <String, Planet>{};
  Planet operator [](String name) =>
      _planets.putIfAbsent(name, () => Planet(name));

  void update(String mainName, String satteliteName) {
    var main = this[mainName];
    var sattelite = this[satteliteName];
    main.children.add(sattelite);
    sattelite.parent = main;
  }

  Planet get centerOfMass => this['COM'];
  static StellarMap parse(String map) {
    var result = StellarMap();
    for (var pair in map.trim().split('\n')) {
      var names = pair.split(')');
      result.update(names.first, names.last);
    }
    return result;
  }

  List<String> path(String fromName, String toName) {
    var fromPlanet = this[fromName];
    var toPlanet = this[toName];
    // need to find a common parent.
    var fromPath = fromPlanet.path;
    var toPath = toPlanet.path;
    var fromIterator = fromPath.iterator;
    var toIterator = toPath.iterator;

    var commonParent = -1;
    while (fromIterator.moveNext() && toIterator.moveNext()) {
      if (fromIterator.current != toIterator.current) {
        break;
      }
      commonParent++;
    }
    return [
      ...fromPath.sublist(commonParent).reversed,
      ...toPath.sublist(commonParent + 1)
    ];
  }

  int get totalOrbits =>
      _planets.values.fold(0, (acc, p) => acc + p.totalOrbits);
}

void main() {
  group('part1', () {
    test('sample', () {
      var map = StellarMap.parse('''COM)B
B)C
C)D
D)E
E)F
...
K)L
''');
      expect(map['L'].totalOrbits, 7);
      expect(map.totalOrbits, 42);
    });

    test('task1', () {
      print(StellarMap.parse(input).totalOrbits);
    });
  });
...
I)SAN
''');
      print(map.path('YOU', 'SAN'));
      expect(map.path('YOU', 'SAN').length - 3, 4);
    });

    test('task1', () {
      print(StellarMap.parse(input).path('YOU', 'SAN').length - 3);
    });
  });
}
