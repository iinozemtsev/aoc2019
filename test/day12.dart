...
List<R> zipmap<R, T, U>(Iterable<T> a, Iterable<U> b, R Function(T, U) map) {
  var result = <R>[];
  var ai = a.iterator;
  var bi = b.iterator;
  while (ai.moveNext() && bi.moveNext()) {
    result.add(map(ai.current, bi.current));
  }
  return result;
}

final listsEqual = ListEquality().equals;

class Vector {
  List<int> coords;
  Vector(this.coords);

  static Vector zeroes(int dimension) =>
      Vector(List.generate(dimension, (_) => 0));
  Vector operator -(Vector other) =>
      Vector(zipmap(coords, other.coords, (a, b) => a - b));
  Vector operator +(Vector other) =>
      Vector(zipmap(coords, other.coords, (a, b) => a + b));

  Vector get sign => Vector(coords.map((c) => c.sign).toList());
  @override
  String toString() => '(${coords.join(', ')})';

  @override
  bool operator ==(dynamic other) =>
      other is Vector && listsEqual(coords, other.coords);
}

final zero3d = Vector.zeroes(3);
...
class Body {
  ...
  Vector acceleration(Body other) => (other.position - position).sign;
  Vector totalAcceleration(List<Body> others) => others
      .where((o) => o != this)
      .fold(zero3d, (a, b) => a + acceleration(b));

  void updateVelocity(List<Body> others) =>
      velocity += totalAcceleration(others);
  void updatePosition() => position += velocity;

  int get kineticEnergy => velocity.coords.fold(0, (a, b) => a + b.abs());
  int get potentialEnergy => position.coords.fold(0, (a, b) => a + b.abs());
  int get totalEnergy => kineticEnergy * potentialEnergy;

  @override
  String toString() => '[$position, $velocity]';

  Body projection(int coord) =>
      Body(Vector([position.coords[coord]]), Vector([velocity.coords[coord]]));
}
...
class System {
  ...
  List<Vector> toVectors() =>
      bodies.expand((b) => [b.position, b.velocity]).toList();

  int get totalEnergy =>
      bodies.map((b) => b.totalEnergy).fold(0, (a, b) => a + b);

  @override
  String toString() => '{\n${bodies.join('\n')}\n}';

  int calculatePeriod() {
    var arity = bodies.first.position.coords.length;
    if (arity == 1) {
      var initialState = toVectors();
      step();
      var stepCount = 1;
      while (!listsEqual(toVectors(), initialState)) {
        step();
        stepCount++;
      }
      return stepCount;
    } else {
      var subsystems = List.generate(arity, (i) => projection(i));
      var periods = subsystems.map((s) => s.calculatePeriod());
      var lcm = periods.reduce((a, b) => a * b ~/ a.gcd(b));
      return lcm;
    }
  }

  System projection(int coord) =>
      System(bodies.map((b) => b.projection(coord)).toList());
}
...
void main() {
  group('part1', () {
    test('sample', () {
      var system = System([
        Body(Vector([-1, 0, 2]), zero3d),
        Body(Vector([2, -10, -7]), zero3d),
        Body(Vector([4, -8, 8]), zero3d),
        Body(Vector([3, 5, -1]), zero3d),
      ]);
      system.steps(1);
      expect(system.bodies[0].velocity.coords, [3, -1, -1]);
      expect(system.bodies[1].velocity.coords, [1, 3, 3]);
      expect(system.bodies[2].velocity.coords, [-3, 1, -3]);
      expect(system.bodies[3].velocity.coords, [-1, -3, 1]);
      expect(system.bodies[0].position.coords, [2, -1, 1]);
      expect(system.bodies[1].position.coords, [3, -7, -4]);
      expect(system.bodies[2].position.coords, [1, -7, 5]);
      expect(system.bodies[3].position.coords, [2, 2, 0]);
    });
    test('sample2', () {
      var system = parse('''<x=-8, y=-10, z=0>
<x=5, y=5, z=10>
<x=2, y=-7, z=3>
<x=9, y=-8, z=-3>
''');
      system.steps(100);
      expect(system.totalEnergy, 1940);
    });
    test('task', () {
      print((parse(input)..steps(1000)).totalEnergy);
    });
  });
...
System parse(String input) => System(input
    .trim()
    .split('\n')
    .map((line) => Body(
        Vector(intPattern
